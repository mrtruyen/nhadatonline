...
class Uploader {
	...
	{	
		//Connect to database
		$this->db = @mysqli_connect(DB_HOST , DB_USER , DB_PASS,DB_NAME) or die("<h1>Error establishing a database connection</h1>");
	...
	public function load($object_id, $user_id = NULL, $full = FALSE, $type = 'all', $type_id=1)
	{
		...
		//echo "SELECT * FROM $this->table WHERE object_id = $object_id $user $type $type_id ORDER BY display_order ASC";
		$result = @mysqli_query($this->db,"SELECT * FROM $this->table WHERE object_id = $object_id ORDER BY id ASC");
		// var_dump($result);
		if (@mysqli_num_rows($result)) {
	...
	public function process_uploaded_files($files, $object_id)
	{
		$result = array();
		$upload_dir = $this->upload_dir;
...
function add_uploader($id, $user_id = null, $type_id) {
	$object_id = $id;
	$user_id = $user_id;
	$type_id = $type_id;
...
function show_files($object_id, $user_id = null, $type_id , $files_type = 'images') {
	global $Uploader;
	...
	if ($files_type == 'images' or $files_type == 'all') 
	{
		...
		if (!empty($files) and is_array($files)) {
			foreach ($files as $key => $file) 
				echo '<a href="../upload/'.$file['file'].'" data-lightbox="gallery-'.$file['object_id'].'" title="'.$file['file'].'"><img src="../upload/thumbnail/'.$file['file'].'" width="80" height="80"></a>';
	...
	if ($files_type == 'files' or $files_type == 'all')
	{
		...
		if (!empty($files) and is_array($files)) {
			...
			foreach ($files as $key => $file)
				echo '<li>
					<a href="files/'.$file['file'].'" title="'.$file['file'].'">
