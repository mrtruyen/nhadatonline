...
if (isset($_POST['action'])) {
	...
	switch ($_POST['action']) {
		case 'save-snapshot':
			...
			$images[0]->title = '';
			$files = $Uploader->process_uploaded_files($images, $_POST['object_id'], $_POST['user_id'], $_POST['type_id']);
...
if (isset($_GET['action'])) {
	...
	switch ($_GET['action']) {
		...
		break;
		case 'load':
			$files = $Uploader->load($_GET['object_id'], null, false,$type = 'all', $_GET['type_id']);
			echo json_encode(array('files' => $files));
		break;
